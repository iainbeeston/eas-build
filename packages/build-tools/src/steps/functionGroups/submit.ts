import { BuildJob, Ios, Platform } from '@expo/eas-build-job';
import {
  BuildFunctionGroup,
  BuildStep,
...
import { resolveEasCommandPrefixAndEnvAsync } from '../../common/easBuildInternal';
...
export function createEasSubmitBuildFunctionGroup(
  ...
  return new BuildFunctionGroup({
    ...
    createBuildStepsFromFunctionGroupCall: (globalCtx, { inputs }) => {
      ...
      const profile = inputs.profile.value ? String(inputs.profile.value) : undefined;
      const isVerboseFastlaneEnabled = inputs.is_verbose_fastlane_enabled.value
        ? String(inputs.is_verbose_fastlane_enabled.value)
        : undefined;
      ...
      const applicationArchiveUrl = inputs.application_archive_url.value
        ? String(inputs.application_archive_url.value)
        : undefined;
...
function validateInputs(inputs: BuildStepInputById): void {
  const providedArgs = [
    inputs.build_id.value,
    inputs.application_archive_url.value,
    inputs.application_archive_path.value,
  ].filter(Boolean);

  if (providedArgs.length > 1) {
    throw new Error(
      `Provide only one of build_id, application_archive_url, or application_archive_path.`
    );
  }
}
...
function createStepsForIosSubmit(
  {
    ...
    applicationArchiveUrl: userApplicationArchiveUrl,
  }: {
    buildId: string;
    profile: string | undefined;
    isVerboseFastlaneEnabled: string | undefined;
    applicationArchivePath: string | undefined;
    applicationArchiveUrl: string | undefined;
  },
  ...
  return [
    ...
    new BuildStep(globalCtx, {
      ...
      fn: async (stepCtx, { outputs }) => {
        if (userApplicationArchivePath) {
          ...
          stepCtx.logger.info(
            `Found existing application artifacts: ${userApplicationArchivePath}`
          );
    ...
    new BuildStep(globalCtx, {
      displayName: 'Determine application url and profile',
      name: 'application_url',
      id: 'application_url',
      outputs: [
        new BuildStepOutput(globalCtx, {
          stepDisplayName: 'Determine application url and profile',
          id: 'application_archive_url',
          required: false,
        }),
        new BuildStepOutput(globalCtx, {
      ...
      fn: async (stepCtx, { env, outputs }) => {
        let applicationArchiveUrl = userApplicationArchiveUrl;
        let buildProfile = userProfile;

        if (userApplicationArchivePath) {
          stepCtx.logger.info('Using user-provided application archive path.');
        } else if (!applicationArchiveUrl) {
          const { cmd, args, extraEnv } = await resolveEasCommandPrefixAndEnvAsync();

          const buildJsonOutput = await spawnAsync(
            cmd,
            [...args, 'build:view', '--json', buildId],
            {
              env: { ...env, ...extraEnv },
              stdio: ['ignore', 'pipe', 'pipe'],
          ...
          const buildJson = JSON.parse(buildJsonOutput.stdout);

          applicationArchiveUrl = buildJson.artifacts.applicationArchiveUrl;
          buildProfile = buildProfile ?? buildJson.buildProfile;
        }
        if (applicationArchiveUrl) {
          stepCtx.logger.info(`Application archive URL: ${applicationArchiveUrl}`);
          outputs.application_archive_url.set(applicationArchiveUrl);
        } else {
          stepCtx.logger.info('Application archive URL not found.');
        }
        if (buildProfile) {
          stepCtx.logger.info(`Build profile: ${buildProfile}`);
          outputs.build_profile.set(buildProfile);
        } else {
          const envBuildProfile = globalCtx.env.EAS_BUILD_PROFILE;
          if (envBuildProfile) {
            stepCtx.logger.info(`Build profile not found. Using ${envBuildProfile}`);
            outputs.build_profile.set(envBuildProfile);
          } else {
            stepCtx.logger.info('Build profile not found.');
          }
        }
      },
    }),
    createDownloadArtifactFunction().createBuildStepFromFunctionCall(globalCtx, {
      id: 'download_artifact',
      callInputs: {
        application_archive_url:
          userApplicationArchiveUrl ?? '${ steps.application_url.application_archive_url }',
      },
    }),
    new BuildStep(globalCtx, {
      ...
      MAYBE_DOWNLOADED_ARTIFACT='\${ steps.download_artifact.artifact_path }'
    ...
    new BuildStep(globalCtx, {
      ...
      env: {
        EAS_USER_IS_VERBOSE_FASTLANE_ENABLED: isVerboseFastlaneEnabled,
      },
      outputs: [
        new BuildStepOutput(globalCtx, {
      ...
      SUBMISSION_CONFIG=$(eas submit:internal --profile '\${ steps.application_url.build_profile }' --platform ios --id ${buildId} 2>/dev/null | jq '.config')
      ...
      if [ -z "$EAS_USER_IS_VERBOSE_FASTLANE_ENABLED" ]; then
        IS_VERBOSE_FASTLANE_ENABLED=$(echo $SUBMISSION_CONFIG | jq -r '.isVerboseFastlaneEnabled')
      else
        IS_VERBOSE_FASTLANE_ENABLED='$EAS_USER_IS_VERBOSE_FASTLANE_ENABLED'
      fi
...
function createStepsForAndroidSubmit(
  ...
  return [
    ...
    new BuildStep(globalCtx, {
      ...
      SUBMISSION_CONFIG=$(eas submit:internal --profile '\${ steps.application_url.build_profile }' --platform android --id ${buildId} 2>/dev/null | jq '.config')

      if [ -z "$EAS_USER_IS_VERBOSE_FASTLANE_ENABLED" ]; then
        if echo "$SUBMISSION_CONFIG" | jq --exit-status '.isVerboseFastlaneEnabled' > /dev/null 2>&1; then
          IS_VERBOSE_FASTLANE_ENABLED=$(echo "$SUBMISSION_CONFIG" | jq -r '.isVerboseFastlaneEnabled')
          set-output is_verbose_fastlane_enabled "$IS_VERBOSE_FASTLANE_ENABLED"
          echo "Setting is_verbose_fastlane_enabled to $IS_VERBOSE_FASTLANE_ENABLED"
        fi
      else
        set-output is_verbose_fastlane_enabled "$EAS_USER_IS_VERBOSE_FASTLANE_ENABLED"
        echo "Setting is_verbose_fastlane_enabled to $EAS_USER_IS_VERBOSE_FASTLANE_ENABLED"
      fi
